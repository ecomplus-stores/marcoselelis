...
toggleButton.addEventListener("click", () => {
  const instantSearchComponent = document.querySelector(
    '[name="InstantSearch"]'
  );
...
window.productListColors = function (_id) {
  $(".product-card[data-product-id=" + _id + "]").each(function () {
    ...
    let itemData =
      colors &&
      colors.attr("data-product") &&
      colors.attr("data-product") != "undefined" &&
      JSON.parse(colors.attr("data-product"));
    ...
    if (
      ...
      for (let i = 0; i < itemData.variationsGrids.colors.length; i++) {
        let image = "";
        let opt = itemData.variations.find((el) =>
          el.specifications.colors.some(
            (color) => color.text == itemData.variationsGrids.colors[i]
          )
        );
...
$("body").on("click", ".product-card_colors", function (event) {
  // Verifica se o clique ocorreu em um botão dentro de .product-card_colors
